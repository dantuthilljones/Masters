...
PeanoReader::PeanoReader(const std::string &file, vtkImplicitFunction* box) {
	std::cout << "Created PeanoReader with box params for file " << std::endl;
	...
	for(uint i = 0; i < lines.size(); i++) {
		...
		if(boost::starts_with(line, "#")) {
			...
			PeanoVariable* variable = new PeanoVariable(variableName, unknowns,
				Cell_Values, cells, mapping, mappings);
			variables.push_back(variable);
			...
			PeanoVariable* variable = new PeanoVariable(variableName, unknowns,
				Vertex_Values, vertices, mapping, mappings);
			variables.push_back(variable);
		} else if(boost::starts_with(line, "dimensions")) {
			std::vector<std::string> split;
			boost::split(split, line, boost::is_any_of(" "));
			...
			for(i++; i < lines.size(); i++) {
				std::string patchLine = lines[i];
				if(boost::starts_with(patchLine, "end patch")) {
					PeanoPatch* patch = new PeanoPatch(patchLines, dimensions,
						patchSize, variables, box);
					if(patch->loaded){
...
PeanoReader::~PeanoReader() {
	...
	//std::cout << "Deleting patch size array\n";
	delete [] patchSize;

	//std::cout << "Deleting patches\n";
